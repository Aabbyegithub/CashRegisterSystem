...
namespace WebServiceClass.QuartzTask
{
    public class TaskSrevice:IBaseService,ITaskService
    {
        ...
        //private async Task<bool> WaitForJobsToFinishAsync(TimeSpan waitTime, CancellationToken cancellationToken)
        //{
        //    var timeout = DateTimeOffset.UtcNow + waitTime;
        //    while (DateTimeOffset.UtcNow < timeout && !_cancellationTokenSource.IsCancellationRequested)
        //    {
        //        if (!_scheduler.CheckExists(null)) // 假设这个方法能检查是否有活动的触发器或作业
        //        {
        //            return true; // 没有活动的作业，可以安全关闭
        //        }

        //        await Task.Delay(100, cancellationToken); // 等待一小段时间再次检查
        //    }
