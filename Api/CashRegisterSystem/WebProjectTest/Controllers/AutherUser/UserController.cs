...
namespace WebProjectTest.Controllers.AutherUser
{
    ...
    public class UserController(IRedisCacheService redisCacheService, ISqlHelper dal, TokenService token) : AutherController(redisCacheService)
    {
        ...
        /// 更新用户信息
        /// </summary>
        /// <param name="lq_User"></param>
        /// <returns></returns>
        [HttpPost]
        [OperationLogFilter("用户>个人信息", "用户修改个人信息", ActionType.Edit)]
        public async Task<ApiResponse<string>> UpdateUserMsgeAsync([FromBody] UserMsg lq_User)
        {
            var OldUserMsg = await dal.Db.Queryable<lq_user>().FirstAsync(a =>a.Id == UserId);
            var LogMsg = "用户修改个人信息！";
            if (!OldUserMsg.IsFirst)
            {
                LogMsg = "用户首次登陆补充个人信息！";
            }
            try
            {
                if (OldUserMsg.UserName != lq_User.UserName)//修改账户名需要判断是否有人已使用该账户名
                { 
                    if (await dal.Db.Queryable<lq_user>().FirstAsync(a => a.UserName == lq_User.UserName && a.OrgId == OrgId) != null)
                    {
                        return Fail<string>("保存失败！账户名重复，已有用户使用该账户名！");
                    }
                }

                await dal.Db.Updateable<lq_user>().SetColumns(a => new lq_user 
                {
                    User = lq_User.User, 
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<int> GetMsgCountAsync()
        {
            var date = await dal.Db.Queryable<lq_sys_msg>().Where(a => a.OrgId == OrgId && a.IsRead == false).CountAsync();
            return date;
        }
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<ApiResponse<List<lq_sys_msg>>> GetMsgAsync()
        {
            var date = await dal.Db.Queryable<lq_sys_msg>().Where(a => a.OrgId == OrgId && a.IsRead == false).ToListAsync();
            date.ForEach(a => { a.IsRead = true; a.UpTime = DateTime.Now; a.UpUserId = UserId; });
            await dal.Db.Updateable(date).ExecuteCommandAsync();
            return Success(date);
        }
    }
}
