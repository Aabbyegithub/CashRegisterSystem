...
namespace WebProjectTest.Controllers.SystemController
{
    ...
    [ApiController]
    [Authorize]
    public class StoreController(IRedisCacheService redisCacheService, IStoreServices _StoreServices) : AutherController(redisCacheService)
    {
        ...
        public async Task<ApiPageResponse<List<sys_store>>> GetStoreListAsync(string? StoreName, string? phone, string? address, int page, int size)
        {
        ...
        public async Task<ApiResponse<bool>> AddStoreAsync([FromBody] sys_store sys_Store)
        {
            if (sys_Store == null)
            {
        ...
        public async Task<ApiResponse<bool>> UpdateStoreAsync([FromBody] sys_store sys_Store)
        {
            if (sys_Store == null)
            {
                return Fail<bool>("商店信息不能为空");
